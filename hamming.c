...
void encode(char *input, int len, FILE *ptr)
{
    ...
    for (int i = 0; i < blocks + 1; i++)
    {
        ...
        { // skip bit 0
            ...
            thisBlock = modifyBit(thisBlock, bits - k, getBit(parityBits, sizeof(block) * 8 - skipped + k));
        }

        // Add overall parity bit (total parity of onCount) //
        thisBlock = modifyBit(thisBlock, bits - skipped, onCount & 1);
